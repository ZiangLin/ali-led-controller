    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (d1 <= colorWheelRadius) {//It's changing color
                    changeColor=true;
                    colorHSV[0] = (float) (Math.toDegrees(Math.atan2(cy1, cx1)) + 180f);
                    colorHSV[1] = Math.max(0f, Math.min(1f, (float) (d1 / colorWheelRadius)));

                    invalidate();
                } else if (x1 >= getWidth() / 2 && d1 >= innerWheelRadius) {//It's changing brightness
                    changeBrightness=true;
                    colorHSV[2] = (float) Math.max(0, Math.min(1, Math.atan2(cy1, cx1) / Math.PI + 0.5f));
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (d <= colorWheelRadius&&changeColor) {
                ...
                } else if (x >= getWidth() / 2 && d >= innerWheelRadius && changeBrightness) {
